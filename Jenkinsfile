pipeline {
  ...
  stages {
    stage('Notify') {
      steps {
        mattermostSend color: "warning", message: "[OWM-Emitter] Build Started - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
      }
    }
    //always run
    stage('Build') {
  ...
  post {
    failure {
       mattermostSend color: "danger", message: "[OWM-Emitter] Build Failure - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
    }
    success{
       mattermostSend color: "good", message: "[OWM-Emitter] Build Success - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
    }
    always {
